...
test.describe('用户注册', () => {
  ...
  test('应该显示注册页面的所有元素', async ({ registerPage }) => {
    ...
    // 验证页面标题
    await expect(registerPage.page).toHaveTitle(/注册/)
    
    // 验证URL
    await expect(registerPage.page).toHaveURL(/\/register/)
  })

  test('应该能够成功注册并自动登录', async ({ registerPage, dashboardPage }) => {
    // 执行注册
  ...
  test('应该在已登录状态下重定向到仪表盘', async ({ registerPage, loginPage, dashboardPage }) => {
    // 先登录
