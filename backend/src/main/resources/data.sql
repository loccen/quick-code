-- 速码网测试数据初始化脚本
-- 适用于Spring Boot JPA自动DDL模式

-- 插入测试用户（如果不存在）
INSERT IGNORE INTO `users` (`username`, `email`, `password`, `nickname`, `status`, `email_verified`, `is_admin`) VALUES
...
-- 插入项目分类（如果不存在）
INSERT IGNORE INTO `project_categories` (`name`, `code`, `description`, `parent_id`, `sort_order`) VALUES
...
-- 插入测试项目（如果不存在）
INSERT IGNORE INTO `projects` (`title`, `description`, `category_id`, `user_id`, `price`, `tech_stack`, `tags`, `status`) VALUES
...
('React Native购物App', '跨平台购物应用，支持商品浏览、购物车、订单管理等功能', 13, 3, 150.00,
 '["React Native", "Redux", "AsyncStorage", "React Navigation"]',
 '["移动应用", "购物", "跨平台", "React Native"]', 1),

('Python数据分析工具', '基于Python的数据分析和可视化工具集，支持多种数据源', 12, 2, 80.00,
 '["Python", "Pandas", "Matplotlib", "Jupyter", "NumPy"]',
 '["数据分析", "Python", "可视化", "工具"]', 1),

('微信小程序商城', '功能完整的微信小程序商城，包含商品展示、购物车、支付等功能', 8, 3, 120.00,
 '["微信小程序", "云开发", "WeUI", "云函数"]',
 '["小程序", "商城", "微信", "云开发"]', 1);
